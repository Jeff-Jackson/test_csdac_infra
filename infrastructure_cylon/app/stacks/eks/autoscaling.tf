...
locals {
  asg_names = data.aws_autoscaling_groups.eks_autoscaling_groups.names
  asg_arns = data.aws_autoscaling_groups.eks_autoscaling_groups.arns
  #asg_names = [for group in module.eks.eks_managed_node_groups : group.node_group_resources]
#  asg_names = [
#  for group in module.eks.eks_managed_node_groups: [
#      for asg in group.node_group_resources.autoscaling_groups: asg.name
#  ]]
}
...
resource "aws_cloudwatch_event_rule" "aws_node_termination_handler_asg" {
  ...
  depends_on = [module.eks.cluster_id, module.eks.eks_managed_node_groups]
  tags = local.tags
}
...
# Creating the lifecycle-hook outside of the ASG resource'ps `initial_lifecycle_hook`
