...
locals {
  ...
  eks_secrets_kms_key_arn_effective = (
    var.eks_secrets_kms_key_arn != null && trimspace(var.eks_secrets_kms_key_arn) != "" ? var.eks_secrets_kms_key_arn : null
  )
}
...
locals {
  ...
  base_roles            = [
    {
      rolearn  = "arn:${var.arn_type}:iam::012555280953:role/service-role/Lambda-CDO-local-role-i4nv3byt"
      username = "cdo-lambda"
    ...
    {
      rolearn  = "${aws_iam_role.lambda_cdo_role.arn}"
      username = "cdo-lambda"
      groups   = ["system:masters"]
    },
    {
      rolearn  = "${aws_iam_role.lambda_cdo_remove_role.arn}"
      username = "cdo-lambda"
      groups   = ["system:masters"]
    },
    {
      rolearn  = "${aws_iam_role.eks_assume_role_policy.arn}"
      username = "cluster-dev-rol"
      groups   = ["system:masters"]
    },
    {
      rolearn  = "arn:${var.arn_type}:iam::<%= expansion(':ACCOUNT') %>:role/EKSCommonServiesTeamTrustRole"
      username = "CommonServices"
      groups   = ["system:commonservices"]
    }
  ]
...
module "eks" {
  ...
  # Do not let the EKS module create its own KMS key
  create_kms_key = false

  cluster_encryption_config = {
    provider_key_arn = local.eks_secrets_kms_key_arn_final
    resources        = ["secrets"]
  }
  ...
  eks_managed_node_groups = {
    ngOne = {
      ...
      iam_role_additional_policies = merge(
        {
          sts    = "arn:${var.arn_type}:iam::<%= expansion(':ACCOUNT') %>:policy/EC2-STS-Asume-Policy"
          vault  = var.csdac_vault_policy_arn
          velero = var.csdac_velero_policy_arn
        },
        var.arn_type == "aws-us-gov" ? {
          base_assume = "arn:${var.arn_type}:iam::<%= expansion(':ACCOUNT') %>:policy/csdac-base-assume-iam-policy"
          s3_readonly = "arn:${var.arn_type}:iam::aws:policy/AmazonS3ReadOnlyAccess"
          cilium      = var.csdac_cilium_policy
        } : {}
      )
      tags = local.tags
  ...
  aws_auth_users = [
    {
      userarn  = "arn:${var.arn_type}:iam::<%= expansion(':ACCOUNT') %>:user/terraform"
      username = "terraform"
      groups   = ["system:masters"]
    }
  ]
...
resource "aws_kms_key" "eks" {
  count = local.eks_secrets_kms_key_arn_effective == null ? 1 : 0
...
locals {
  eks_secrets_kms_key_arn_final = coalesce(
    local.eks_secrets_kms_key_arn_effective,
...
resource "aws_security_group" "additional" {
  ...
  ingress {
    description = "Allow all for 10.220.0.0/16 and 172.16.0.0/12 ports/protocols"
    protocol    = "-1"
    from_port   = 0
    to_port     = 0
    cidr_blocks = ["10.220.0.0/16", "172.16.0.0/12"]
  }
  dynamic "ingress" {
